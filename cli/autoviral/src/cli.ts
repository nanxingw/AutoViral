...
function usage(): string {
  return [
    ...
    "  comp fps <24|25|30|60>        Switch the canvas frame rate in one shot.",
    "    24 recommended — Seedance (the primary generation source) is a constant",
    "    24fps; time fields are all in seconds, so switching fps is lossless.",
    ...
    "  clip move <id> --to-track <trackId> [--offset <seconds>]  Move a clip to",
    "    another lane (kind-guarded) and/or reposition it on the timeline.",
    "  clip duplicate <id> [--offset <seconds>]   Clone a clip (fresh id).",
    "  clip import <path> [--track <trackId>] [--replace-timeline] [--at <seconds>]",
    "    ffprobe a finished/external video + place it on the timeline (registers an",
    "    asset + provenance). --replace-timeline wipes the video lanes first.",
    "  clip reframe <id> --aspect <W:H> [--punch-in <scale>] [--from <s> --to <s>]",
    "    Auto smart-crop to a new aspect (centered crop + optional punch-in zoom).",
    "  clip detach-audio <id>          Split the clip's own audio onto a new audio",
    "    lane (decouple narration/original sound from the video).",
    "  clip mask <id> --shape <rect|ellipse> [--x N --y N --w N --h N] [--feather N]",
    "    | --preset <letterbox-2.35|…> | --none   Add/replace/clear a shape mask.",
    "  clip keyframe add|set|remove|move <id> --property <p> [--at <s>] [--value V]",
    "    [--easing <linear|easeIn|easeOut|easeInOut|cubic-bezier(x1,y1,x2,y2)>]",
    "    Hand-author a property curve (opacity/scale/x/y/rotation/volume/speed).",
    "  clip effects add|remove|reorder|toggle|set <id> [...]   Ordered effect stack",
    "    (grade/blur/vignette/grain). Full flags: `autoviral docs _shared/03-cli-reference`.",
    "  clip remove <id> [--ripple]     Remove a clip. --ripple also slides every",
    "    later clip on the lane left to close the gap (no dead air).",
    ...
    "  captions generate --script <file> [--max-cjk-chars <N>]  Align ASR word",
    "    TIMING onto your ground-truth script text (never the mis-heard transcript),",
    "    split CJK into ≤N-char lines (default 14), write an overlay CaptionModel.",
    "    --language is OPTIONAL — auto-detect works (no more 500 without it).",
