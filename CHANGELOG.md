...
## [Unreleased] — `refactor/agentic-terminal` (2026-05-14)

Major product reframing: **AutoViral is now a creator workstation + an agent-agnostic operating protocol**, not "a Claude-Code-driven video tool". Any CLI agent (claude / codex / kimi / gemini / aider / cursor-agent) running in the Studio terminal panel can drive AutoViral by loading the operator-manual skill and calling the `autoviral` CLI.
...
- **Bridge HTTP+WebSocket protocol v1** (`/api/bridge/v1/*` + `/ws/bridge/:workId` + `/ws/terminal/:workId`). Loopback-only, cross-origin upgrades rejected. Spec at `docs/superpowers/specs/2026-05-14-agentic-terminal-bridge-protocol.md`.
...
- Plan + spec under `docs/superpowers/{plans,specs}/2026-05-14-agentic-terminal-*.md`.
