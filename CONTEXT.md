...
| **research/** | Per-work directory for free-form, on-demand research notes. It remains part of the work even though the product-wide discovery/trends surface is retired. |
...
| **Chat panel** | The non-technical-user-facing agent surface. Streaming markdown chat (pneuma-inspired), `<viewer-context>` envelope auto-prepends user focus state, `<viewer-action/>` tag from agent drives Studio (seek/select/scroll), checkpoint rollback for safety. Backend: spawns a chat-CLI subprocess per session via the `ChatBackend` seam — **claude** (`claude -p`) or **codex** (`codex exec --json`), pinned per-session at creation ([ADR-013](docs/adr/ADR-013-chat-multi-backend.md)). |
...
| **context channel** | Agent-facing aggregator that emits work + focus + composition + profile as a single JSON snapshot. CLI: `autoviral context [--watch]`. |
...
4. **Terminal-panel agents are skill-agnostic.** Any CLI agent (Claude / Codex / Kimi / Gemini / Aider) should drive AutoViral identically via the `autoviral` CLI + bridge. No agent-specific paths in the CLI or backend. The project-level entry for non-Claude agents is [`AGENT.md`](AGENT.md) — it is **CLAUDE.md's same-role backend counterpart**: a thin pointer to the shared `CLAUDE.md` / `CONTEXT.md` rules plus the backend-specific deltas (build/test commands, the Terminal-vs-Chat surface split, the OpenRouter-only gateway). Note the scope of this invariant: it constrains the **Terminal** surface only. The **Chat** panel was claude-code-only through v0.1.7 (spawned `claude -p`), an intended-default gap per [ADR-005](docs/adr/ADR-005-dual-chat-entry-layout.md). **As of v0.1.8 the Chat panel is multi-backend** ([ADR-013](docs/adr/ADR-013-chat-multi-backend.md)): a `ChatBackend` seam behind the single spawnCli chokepoint lets each chat session pick **claude** or **codex** at creation, pinned per-session (resume lineages are not interchangeable, so the backend is immutable once a session has a conversation). codex usage is token-only (no local USD guess); auth is self-managed per backend — codex ships a login probe (`checkCodexAuth` reads `<codexHome>/auth.json`) whose intent is to nudge unauthenticated users to `codex login`, but the spawn-time gate that would surface that guidance *instead of* spawning is **not yet wired into the bridge** (a follow-up on the C3/C4 backend work; today an unauthenticated codex session still gets the CLI's own error — see [ADR-013](docs/adr/ADR-013-chat-multi-backend.md) §4).
...
8. **Honesty over a full-looking UI.** Never fabricate data, never promise data the tool cannot obtain. On-demand research must distinguish sourced facts from inference, and missing evidence must stay visibly missing rather than being filled with estimates. Retiring an unsupported product surface is preferable to maintaining a misleading placeholder. *Rationale: [ADR-014](docs/adr/ADR-014-retire-discovery-and-analytics.md).*

9. **Tests must not write the real `~/.autoviral`.** Code that resolves its data dir from `os.homedir()` (for example, cover routes) is NOT isolated by `withTempDataDir` (which only overrides `AUTOVIRAL_DATA_DIR`). Setting `process.env.HOME` is fragile (cached `homedir()`). Isolate by mocking the module: `vi.mock("node:os", … homedir: () => fakeHome)` (not `vi.spyOn` — ESM named exports are non-configurable getters).
...
- **2026-07-11** · PRD-0013 S4 — retired the product-wide Explore / Analytics surfaces, their dedicated API / CLI paths, and both background collectors. Removed their current domain vocabulary and code-map entries; retained `topicHint`, per-work `research/`, generic on-demand research, and cost ledger. New decision: [ADR-014](docs/adr/ADR-014-retire-discovery-and-analytics.md); ADR-010 / ADR-011 are superseded.
...
- **2026-07-03** · v0.1.8 ([PRD-0010](docs/prd/0010-v0.1.8-chat-codex-cost-canvas.md)) — Chat 多后端. **invariant 4 updated**: the Chat panel is no longer claude-only — a `ChatBackend` seam behind the spawnCli chokepoint lets each session pick claude or codex at creation, pinned per-session (resume lineages not interchangeable → immutable after first turn); codex usage is token-only; auth self-managed per backend. At release time coach/trends remained claude-only; those surfaces were later retired by [ADR-014](docs/adr/ADR-014-retire-discovery-and-analytics.md). New decision: [ADR-013](docs/adr/ADR-013-chat-multi-backend.md).
