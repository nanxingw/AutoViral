...
> ⚠️ **ARCHIVED — pre-D3 issue log (2026-03 ~ 2026-04 早期)**
> 本文档收集了旧版本"四步流水线"架构下的 bug 与改进点。其中相当一部分（如 BUG-3 阶段对话历史丢失、ARCH-4 素材生成阶段缺乏环境感知）已经在 D3 重构（modules-as-capabilities）中**结构性消除**——不再有"阶段切换"，也不再有 `pipeline/advance` 端点。本文保留作为历史参考。新版本的开放议题请走 GitHub Issues。
...
**相关文件**：
- `skills/autoviral/modules/assembly/scripts/caption_generate.py` — ASS 字幕生成（未被调用）
- `skills/autoviral/modules/assets/scripts/font_manager.py` — 字体管理（已工作）
- `skills/autoviral/modules/assembly/SKILL.md` — 合成阶段 Skill（需补充强制字幕规范）
- `skills/autoviral/modules/assembly/capabilities/subtitle-aesthetics.md` — 字幕美学参考（需更新）
