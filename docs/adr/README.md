...
| [ADR-010](ADR-010-grounded-coach-persona.md) | Grounded coach — 灵感页第二个持久 agent persona（workless `coach_main` 会话、sidecar 持久、session 级 model、复用聊天栈）（v0.1.5 / PRD-0006 S6-S8） | Superseded by ADR-014 | 2026-06-08 |
| [ADR-011](ADR-011-douyin-collector-managed-venv-scrape.md) | Douyin 采集器 — 托管 venv（f2+browser_cookie3）经用户浏览器 session 抓本人数据；人口属性删卡非推迟；本地优先 secrets 明文=接受风险（v0.1.5 / PRD-0006 S4-S5） | Superseded by ADR-014 | 2026-06-08 |
...
| [ADR-013](ADR-013-chat-multi-backend.md) | Chat 多后端架构 — ChatBackend 接缝（spawn+parse 归一）、per-session backend 钉定、resume lineage 不互通首轮后不可变、认证自理、codex token-only（v0.1.8 / PRD-0010 C2·C3·C4） | Accepted | 2026-07-03 |
| [ADR-014](ADR-014-retire-discovery-and-analytics.md) | 聚焦内容生产，退役 Explore / Analytics、专属 API / CLI 与两套后台采集；保留 topicHint、每作品 research 与 cost ledger（PRD-0013 S4） | Accepted | 2026-07-11 |
