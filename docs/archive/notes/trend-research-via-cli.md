...
> **归档说明（2026-07-11）：** Explore / Analytics 与其专属趋势采集链路已由 [ADR-014](../../adr/ADR-014-retire-discovery-and-analytics.md) 退役。本文仅保留历史背景，不再描述受支持的产品能力，其中“估算数据”等旧建议不得复用。当前作品需要调研时，由 agent 按需执行通用 Web research，并将笔记保存到该作品的 `research/` 目录；入口说明见[现行参考页](../../reference/experience/trend-research-via-cli.md)。
