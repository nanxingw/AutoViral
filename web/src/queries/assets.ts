...
const PIPELINE_INTERNAL: RegExp[] = [
  ...
  // The optional `-na` suffix marks a detached-source (video-only) cache
  // variant (S5 review fix #4 — sourceAudio.enabled folded into the key).
  /^output\/clip-[^/]*-speed-\d+(-fps\d+)?(-na)?\.mp4$/i, // static speed-ramp pre-pass cache
  ...
  /^output\/clip-[^/]*-speedvar-[0-9a-f]+(-na)?\.mp4$/i, // variable-speed concat pre-pass cache
