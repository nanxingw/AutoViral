...
>
> **`data:` anchor probe-verified 2026-06-15** (the 06-10 batch only ever sent http-URL anchors — the `data:` inline path was code-correct but never hit the real API until now): a local image passed as a work-relative `firstFrame` → server inlines it to a `data:image/...;base64,…` URI → **OpenRouter Seedance accepts it and renders** (a 64×64 `data:` anchor + explicit `16:9` produced a real 864×496 / 24fps / 4s clip — also re-confirming ① with a `data:` anchor: the square anchor did **not** lock the output square). So the OpenRouter cookbook's "use a stable downloadable URL" is a *recommendation*, not a hard requirement — `data:` works too.
>
> ⚠️ **Anchor reachability** — Seedance fetches the anchor **server-side, from OpenRouter's servers**, not from your machine. Two `firstFrame` forms reach it: (a) a **work-relative asset path** (the route inlines it to a `data:` URI — preferred, always works) or (b) a **publicly downloadable** `https://` URL. A `http://localhost…` / `http://127.0.0.1…` / private-LAN URL fails with `400 … resource download failed` because OpenRouter's servers can't reach your box. **Never hand Seedance a localhost URL as the anchor** — pass the workspace path and let the route inline it.
