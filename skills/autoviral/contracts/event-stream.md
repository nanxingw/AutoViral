...
| `settleReason` | string? | Present only on a **synthesized** terminal state (host process exited while the task was still live) — e.g. `cli_exit` / `superseded`. A task that reached a natural terminal via a CLI frame has no `settleReason`. Preserved (not cleared) when a `stopped` task is later upgraded to `orphaned` by harvest. |
| `harvest` | object? | Present only on an `orphaned` terminal (journal harvest). `{ runId, completedAgents, startedAgents, journalPath, resultSummary? }` — how many sub-agents actually finished (deduped by journal `key`, so a resume that re-ran an agent under a new id is **not** double-counted) out of how many started, plus the run id / journal path as resume + audit leads and (`resultSummary`, W4.5 L10) a ≤120-char summary of the last agent result. The "loss is visible, spend is queryable" surface. |
| `ts` | number | Broadcast epoch ms. |

**Identity.** The frame carries `workId + sessionId + taskId + generation`. The bridge keys its registry by `generation :: taskId` (per chat session); the **client** keys by `workId :: sessionId :: generation :: taskId` (W4.5 H5) so two different works that reuse the same session id (both default `s_1`) never cross-contaminate.

**State machine.** A task is created `running`, may go `pending-settle` (dropped from the live snapshot but no terminal frame yet), and settles into one terminal: `completed` \| `failed` \| `killed` \| `stopped` \| `orphaned`. Terminal is **monotonic** — a late non-terminal frame never revives it. The only allowed terminal→terminal moves are a small whitelist (identical on server `TERMINAL_OVERRIDE_ALLOWED` and client `shouldReplace`): CLI-authoritative `killed`→`stopped`, and journal-harvest `stopped`→`orphaned` / `killed`→`orphaned` (S7). An idempotent same-terminal re-broadcast (e.g. a second `completed` carrying a fresher `summary`/`usage`) **overwrites the data but fires no new toast** (W4.5 H2). Everything else (e.g. `completed`→`killed`, or harvest trying to `orphaned` an already-`completed` task) is rejected and re-affirms the existing terminal. `orphaned` is produced **only** by the internal journal-harvest path after the host process died, never speculatively — an upstream CLI frame carrying `status: "orphaned"` is **rejected** (`orphaned_requires_harvest`, W4.5 M9).

**Journal harvest → `orphaned` (S7 / W4.5 H3·M6·M7).** After the host CLI process exits or a destructive kill settles its tasks, the server harvests every **non-`completed` terminal** (`killed`/`stopped`) `local_workflow` task of that generation — recomputed from the registry snapshot, so a task that was CLI-marked `killed` *before* the process exited (ceiling force-kill) is harvested too, not only the ones just synthesized (H3). It reads the on-disk workflow journal (`~/.claude/projects/<slug(cwd)>/<cliSessionId>/subagents/workflows/<runId>/journal.jsonl`), and when several `<runId>` dirs coexist it attributes to each task the run whose journal mtime falls inside **that task's** process-lifecycle window; if no run uniquely matches it conservatively leaves the task at its terminal rather than cross-number it (M6). The single-run convenience path takes the latest-mtime run and, **if that run's journal is corrupt, returns `null` without falling back to an older run** (M7). On a match it counts the completed sub-agents (deduped by journal `key`), upgrades the task to `orphaned` with a `harvest` object, and **broadcasts one more `ui-workflow`**. This is best-effort and asynchronous: it never blocks the settle/broadcast path, and a missing/corrupt/ambiguous journal simply leaves the task at its terminal (graceful degradation). Harvest only makes the crash's already-completed work **visible + costed** — it does **not** auto-resume or re-run anything.
