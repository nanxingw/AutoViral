...
| `harvest` | object? | Present only on an `orphaned` terminal (journal harvest). `{ runId, completedAgents, startedAgents, journalPath }` — how many sub-agents actually finished (deduped by journal `key`, so a resume that re-ran an agent under a new id is **not** double-counted) out of how many started, plus the run id / journal path as resume + audit leads. The "loss is visible, spend is queryable" surface. |
| `ts` | number | Broadcast epoch ms. |

**State machine.** A task is created `running`, may go `pending-settle` (dropped from the live snapshot but no terminal frame yet), and settles into one terminal: `completed` \| `failed` \| `killed` \| `stopped` \| `orphaned`. Terminal is **monotonic** — a late non-terminal frame never revives it. The only allowed terminal→terminal moves are a small whitelist: CLI-authoritative `killed`→`stopped`, and journal-harvest `stopped`→`orphaned` / `killed`→`orphaned` (S7). Everything else (e.g. `completed`→`killed`, or harvest trying to `orphaned` an already-`completed` task) is rejected and re-affirms the existing terminal. `orphaned` is produced **only** by journal harvest after the host process died, never speculatively.

**Journal harvest → `orphaned` (S7).** After the host CLI process exits and its still-live tasks are settled to `stopped`, the server reads the on-disk workflow journal (`~/.claude/projects/<slug(cwd)>/<cliSessionId>/subagents/workflows/<runId>/journal.jsonl`) for each settled `local_workflow` task, counts the completed sub-agents (deduped by journal `key`), upgrades the task to `orphaned` with a `harvest` object, and **broadcasts one more `ui-workflow`**. This is best-effort and asynchronous: it never blocks the settle/broadcast path, and a missing/corrupt journal simply leaves the task at `stopped` (graceful degradation). Harvest only makes the crash's already-completed work **visible + costed** — it does **not** auto-resume or re-run anything.
