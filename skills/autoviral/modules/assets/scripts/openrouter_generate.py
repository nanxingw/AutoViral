#!/usr/bin/env python3
"""
OpenRouter 图片生成工具（唯一生图工具）
通过 OpenRouter API 调用 OpenAI GPT-5.4 Image 生成高质量图片。

默认模型:
  - openai/gpt-5.4-image-2 (唯一支持的生图模型)
                    ...
                    with open(candidate) as f:
                        for line in f:
                            ...
                            if "=" in line:
                                ...
                                # 按 search_roots 优先级：先遇到的不被后续覆盖
                                merged.setdefault(key.strip(), value.strip())
                break  # 每个 root 只取最近的一个 .env，不继续向上
...
def generate_image(
    ...
    Args:
        ...
        output_path: 输出文件路径
        model: 模型 ID
        aspect_ratio: 宽高比 (1:1, 3:4, 9:16, 16:9 等)
        image_size: 分辨率等级 (1K, 2K)
        seed: 随机种子（用于可重复生成）
...
示例:
  ...
  # 抖音封面（9:16 全屏竖图，2K）
  %(prog)s --prompt "..." --aspect-ratio 9:16 --image-size 2K --output cover.png
...
宽高比选项:
  ...
  扩展(极端长宽，兼容性依模型而定): 1:4, 4:1, 1:8, 8:1
