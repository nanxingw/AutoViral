...
def check_providers(env_vars: dict[str, str]) -> dict:
    ...
    # OpenRouter (OpenAI GPT-5.4 Image) — 唯一图片生成通道
    openrouter_key = env_vars.get("OPENROUTER_API_KEY", "")
    openrouter_ready = bool(openrouter_key)
    providers.append({
        "name": "openrouter",
        "display_name": "OpenRouter (OpenAI GPT-5.4 Image)",
        "available": openrouter_ready,
        "supports_image": True,
        "supports_video": False,
        "missing_keys": ["OPENROUTER_API_KEY"] if not openrouter_key else [],
        "script": "openrouter_generate.py",
        "note": "唯一图片生成通道，支持 1K/2K 分辨率/宽高比/seed/图生图，模型: openai/gpt-5.4-image-2",
        "models": [
            "openai/gpt-5.4-image-2 (唯一支持)",
        ],
        "features": ["aspect_ratio", "image_size (1K/2K)", "seed", "ref-image", "temperature"],
    })

    # 即梦 API (Jimeng / VolcEngine) — 仅视频生成备用（图片能力已禁用）
    ...
    providers.append({
        ...
        "note": "视频生成备用（Dreamina CLI 不可用时）。图片生成已全部迁移到 openai/gpt-5.4-image-2。",
    })
