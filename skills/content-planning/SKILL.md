...
1. **像朋友对着你说话** — 一个人对着镜头，用日常语气讲述，就像 dating coach 在跟你 1v1 聊天。越日常越好，让人有亲切感。
2. **场景极简** — 固定一个场景（卧室、客厅、咖啡厅、车里），不需要频繁切换场景。背景要生活化，不要刻意布景。
3. **镜头语言极简** — 主要用固定中景（胸部以上），偶尔切特写强调重点。不需要推拉摇移等复杂运镜。
4. **节奏靠语言驱动** — 不靠画面变化维持注意力，靠说话的内容、语气、停顿来抓人。
5. **文字叠加是核心** — 大量使用屏幕文字辅助表达：标题文字、重点金句、列表编号。很多观众静音看，文字就是内容。
6. **BGM 低调** — 轻音乐垫底即可，不要抢过说话的声音。

**口播风格的分镜应该这样写：**

```
| 镜号 | 场景描述 | 时长 | 旁白/字幕 | 镜头 |
| 01 | 人物对镜说话，卧室背景，自然光 | 3s | [Hook 文字 + 口播] | 固定中景 |
| 02 | 同上 | 5s | [观点1 + 屏幕文字] | 固定中景 |
| 03 | 同上，略微推近 | 5s | [观点2 + 屏幕文字] | 缓推特写 |
| ...  | 重复，保持同一场景 | ... | ... | 固定/微推 |
```

**不需要为每个镜头写不同的场景** — 就是同一个人在同一个地方说话，内容靠字幕和口播推进。
