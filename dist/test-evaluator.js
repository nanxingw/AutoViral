...
// ── Evaluator ──────────────────────────────────────────────────────────────
export async function evaluateWork(workId, contentType) {
    ...
    const processScore = Math.round((processChecks.filter(c => c.passed).length / Math.max(processChecks.length, 1)) * 100);
    const outputScore = Math.round((outputChecks.filter(c => c.passed).length / Math.max(outputChecks.length, 1)) * 100);
...
// ── Process Checks ─────────────────────────────────────────────────────────
async function checkProcess(workId) {
    const checks = [];
    ...
    for (const [key] of steps) {
        ...
        checks.push({
            name: `步骤 ${key} 有聊天记录`,
            passed: !!(history && history.blocks?.length > 0),
            detail: history ? `${history.blocks?.length ?? 0} blocks` : "无记录",
        });
    }
    return checks;
}
// ── Output Checks ──────────────────────────────────────────────────────────
async function checkOutput(workId, contentType) {
    const checks = [];
    try {
        const assets = await listAssets(workId);
        const images = assets.filter((a) => /\.(png|jpe?g|webp)$/i.test(a));
        const videos = assets.filter((a) => /\.(mp4|mov|webm)$/i.test(a));
        // Image count
        ...
        // Publish text exists
        const hasPublishText = assets.some((a) => a.includes("publish-text"));
        checks.push({
            name: "发布文案存在",
        ...
        if (hasPublishText) {
            try {
                ...
                const textPath = assets.find((a) => a.includes("publish-text"));
...
async function evaluateQuality(workId) {
    try {
        // Gather content for evaluation
        const workDir = join(homedir(), ".autoviral", "works", workId);
        const assets = await listAssets(workId);
        let publishText = "";
        const textAsset = assets.find((a) => a.includes("publish-text"));
        if (textAsset) {
            publishText = await readFile(join(workDir, textAsset), "utf-8").catch(() => "");
        }
        let planText = "";
        const planAsset = assets.find((a) => a.includes("content_plan"));
        if (planAsset) {
            planText = await readFile(join(workDir, planAsset), "utf-8").catch(() => "");
        }
        const imageCount = assets.filter((a) => /\.(png|jpe?g|webp)$/i.test(a)).length;
        if (!publishText)
            return [];
...
//# sourceMappingURL=test-evaluator.js.map