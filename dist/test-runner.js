...
import { createWork, getWork } from "./work-store.js";
...
export async function runPipeline(wsBridge, config) {
    ...
    log("info", "server", "test_run_started", undefined, { runId, config: config });
    try {
        // 1. Create work
        ...
        for (const stepKey of stepKeys) {
            ...
            try {
                ...
                if (completion.error)
                    stepResult.error = completion.error;
            }
            catch (err) {
            ...
            if (stepResult.status === "failed" || stepResult.status === "timeout") {
                ...
                if (!nextPending)
                    break; // All done or no more steps
            }
        }
        // 3. Final status
...
async function waitForStepCompletion(wsBridge, workId, stepKey, prompt, model, timeout) {
    return new Promise((resolve, reject) => {
        ...
        const cleanup = wsBridge.onSessionEvent(workId, (event, data) => {
            if (settled)
                return;
            if (event === "tool_use") {
...
async function generateUserReply(agentText, stepKey, workTitle) {
