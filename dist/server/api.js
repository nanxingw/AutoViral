...
import { listWorks, getWork, createWork as storeCreateWork, updateWork as storeUpdateWork, deleteWork as storeDeleteWork, listAssets, getAssetPath, saveStepHistory, loadStepHistory, } from "../work-store.js";
import { MemoryClient } from "../memory.js";
import { getProvider, getDefaultProvider, listProviders } from "../providers/registry.js";
import { listSharedAssets, getSharedAssetPath } from "../shared-assets.js";
...
import { runPipeline, getRunStatus, listRuns, getRunReport } from "../test-runner.js";
import { evaluateWork } from "../test-evaluator.js";
export const apiRoutes = new Hono();
// ── Python script runner for real-time trend data ────────────────────────────
const execFileAsync = promisify(execFile);
async function runTrendScript(platform) {
...
let wsBridge = null;
export function setWsBridge(bridge) {
    wsBridge = bridge;
}
// ── Status & Config ─────────────────────────────────────────────────────────
// GET /api/status
...
apiRoutes.put("/api/config", async (c) => {
    ...
    if (body.jimengAccessKey !== undefined) {
        if (!config.jimeng)
            config.jimeng = { accessKey: "", secretKey: "" };
        config.jimeng.accessKey = body.jimengAccessKey;
    }
    if (body.jimengSecretKey !== undefined) {
        if (!config.jimeng)
            config.jimeng = { accessKey: "", secretKey: "" };
        config.jimeng.secretKey = body.jimengSecretKey;
    }
    if (body.openrouterKey !== undefined) {
        config.openrouter = { apiKey: body.openrouterKey };
    }
    if (body.researchEnabled !== undefined) {
        if (!config.research)
            config.research = { enabled: false, schedule: "0 9 * * *", platforms: ["douyin", "xiaohongshu"] };
        config.research.enabled = body.researchEnabled;
    }
    if (body.researchCron !== undefined) {
        if (!config.research)
            config.research = { enabled: false, schedule: "0 9 * * *", platforms: ["douyin", "xiaohongshu"] };
        config.research.schedule = body.researchCron;
    }
    ...
    if (body.douyinUrl !== undefined) {
        if (!config.analytics)
            config.analytics = { douyinUrl: "", collectInterval: 60, enabled: true };
        config.analytics.douyinUrl = body.douyinUrl;
    }
    if (body.memorySyncEnabled !== undefined) {
        if (!config.memory)
            config.memory = { apiKey: "", userId: "autoviral-user", syncEnabled: false };
        config.memory.syncEnabled = body.memorySyncEnabled;
    }
    ...
    try {
        ...
        const enriched = await Promise.all(works.map(async (w) => {
            try {
                ...
                const finalVideo = assets.find((a) => /\.(mp4|mov|webm)$/i.test(a) && /final/i.test(a));
                if (finalVideo) {
                ...
                const outputImage = assets.find((a) => /\.(png|jpe?g|webp|gif)$/i.test(a) && a.startsWith("output/"));
                if (outputImage) {
                    return { ...w, coverImage: `/api/works/${w.id}/assets/${outputImage.split("/").map(encodeURIComponent).join("/")}` };
                }
                // 3. Any asset image
                const firstImage = assets.find((a) => /\.(png|jpe?g|webp|gif)$/i.test(a));
                if (firstImage) {
...
apiRoutes.post("/api/works/:id/assets/upload", async (c) => {
    ...
    const subdir = body.subdir ?? "images";
...
apiRoutes.get("/api/analytics/creator", async (c) => {
    ...
    if (!latest)
        return c.json({ configured: false, data: null });
...
function runCliBrief(prompt, timeoutMs = 60000) {
    return new Promise((resolve, reject) => {
        ...
        catch { } reject(new Error("Timeout")); }, timeoutMs);
    });
}
async function researchTrends(platforms) {
    const collected = [];
    const errors = [];
    ...
    for (const platform of platforms) {
        ...
        try {
            ...
            await writeFile(join(trendsDir, `${dateStr}.yaml`), yaml.dump(data, { lineWidth: -1 }), "utf-8");
...
apiRoutes.post("/api/trends/refresh", async (c) => {
    try {
        ...
        const platforms = body.platforms ?? ["xiaohongshu", "douyin"];
...
apiRoutes.post("/api/trends/refresh-stream", async (c) => {
    if (!wsBridge)
        return c.json({ error: "WsBridge not initialized" }, 503);
    try {
        const body = await c.req.json().catch(() => ({}));
        ...
        const config = await loadConfig();
        const reqInterests = body.interests ?? config.interests ?? [];
        const interests = reqInterests;
...
apiRoutes.post("/api/works/:id/session", async (c) => {
    ...
    try {
        ...
        const prompt = isEn ? [
            ...
            `Target platforms: ${work.platforms.map((p) => typeof p === "string" ? p : p.platform).join(", ")}.`,
            ...
            `目标平台: ${work.platforms.map((p) => typeof p === "string" ? p : p.platform).join(", ")}。`,
...
apiRoutes.post("/api/works/:id/step/:step", async (c) => {
    ...
    try {
        ...
        if (!pipelineStep)
            return c.json({ error: `Unknown pipeline step: ${step}` }, 404);
        ...
        const promptParts = [
            ...
            `Platforms: ${work.platforms.map((p) => typeof p === "string" ? p : p.platform).join(", ")}.`,
        ...
        if (step === "material-search" && work.videoSearchQuery) {
            promptParts.push(`Execute the "视频搜索" step.`, `The user wants to find existing videos from the web. Search query: "${work.videoSearchQuery}"`, ``, `## CRITICAL: Five-Dimension Constraint Analysis`, `Before searching, you MUST parse the search query into 5 dimensions and treat them as hard constraints:`, `1. **Absolute Subject & Physical Motion** — Who/what must appear, doing what? Subject must be visible EVERY SECOND.`, `2. **Environment & Emotional Lighting** — What scene/setting? What light mood?`, `3. **Optics & Camera** — What shot type, angle, movement?`, `4. **Timeline & State Evolution** — Duration required? Speed (normal/slow/fast)? How does the subject change over time?`, `5. **Aesthetic Medium & Rendering** — Live action / animation / 3D? Color tone? Resolution?`, ``, `Parse the query "${work.videoSearchQuery}" into these 5 dimensions first. State which are hard constraints (explicitly mentioned) vs soft constraints (inferred). Then search accordingly.`, `ALL returned videos must satisfy ALL hard constraints. If a video violates any (e.g. subject disappears mid-way), discard it.`, ``, `## Instructions`, `1. Search the web for 3 matching videos using WebSearch.`, `2. For each video found, download it WITH AUDIO using yt-dlp and save to the work assets directory.`, `   - First check if yt-dlp is available: \`which yt-dlp || pip3 install yt-dlp\``, `   - Download command (MUST use this to get audio+video merged):`, `     \`yt-dlp -f "bestvideo[height<=720]+bestaudio/best[height<=720]" --merge-output-format mp4 -o "/path/to/option-01.mp4" "VIDEO_URL"\``, `   - Save videos to the work assets directory. Find the path with:`, `     \`curl -s http://localhost:3271/api/works/${work.id} | python3 -c "import sys,json; w=json.load(sys.stdin); print(w.get('path',''))" || echo "$HOME/.autoviral/works/${work.id}/assets/clips"\``, `   - Save as: option-01.mp4, option-02.mp4, option-03.mp4`, `   - NEVER use plain curl to download videos — it will only get the video stream without audio.`, `3. Present the 3 options to the user using markdown video links so they display as inline players:`, `   - Use this format: \`[Video Title](/api/works/${work.id}/assets/clips/option-01.mp4)\``, `   - The .mp4 link format will render as an inline video player in the chat.`, `4. Ask the user to choose one of the 3 videos.`, `5. After the user selects, rename/copy the chosen video as the primary clip and mark this step as done:`, `   \`curl -X POST http://localhost:3271/api/works/${work.id}/pipeline/advance -H "Content-Type: application/json" -d '{"completedStep":"material-search","nextStep":"research"}'\``, ``, `IMPORTANT:`, `- Video files MUST have audio. Always use yt-dlp with audio merging, never plain curl/wget.`, `- Files must be actually downloaded and saved as assets so the inline player can play them.`);
        }
            ...
            const cat = work.contentCategory;
            // Load user interests and competitors for topic relevance
            const config = await loadConfig();
            const userInterests = (config.interests ?? []);
            const douyinUrl = config.douyinUrl ?? "";
            ...
            promptParts.push(`Execute the "${pipelineStep.name}" step of the pipeline.`, `Produce output appropriate for this step. Be thorough and creative.`);
            if (step === "assembly" && work.type === "short-video") {
                ...
                promptParts.push(``, `## REQUIRED: Generate Narration Audio`, ``, `Before assembling the final video, you MUST generate a narration voiceover audio file.`, ``, `**Voice selection:** You will use a **${voiceConfig.label}** for this narration.`, `Before starting, tell the user in the chat: "${isEn
                    ...
                    : `我将使用${voiceConfig.label}来生成旁白。如果你希望换一种声音（比如女声、不同口音等），请在我开始前告诉我。`}"`, ``, `**How to generate:**`, `Use the \`edge-tts\` command to convert the narration script to audio:`, `\`\`\`bash`, `edge-tts --text "YOUR NARRATION TEXT HERE" --voice ${voiceConfig.voice} --write-media <work_dir>/assets/clips/narration.mp3`, `\`\`\``, ``, `**Voice options (if user requests a change):**`, isEn ? [
                ...
                ].join("\n"), ``, `**Steps:**`, `1. Write the narration script based on the content plan`, `2. Tell the user the voice you'll use and ask for confirmation`, `3. After confirmation (or immediately if user says to proceed), run edge-tts to generate the audio file`, `4. Merge the narration audio with the video clips using ffmpeg`, ``);
                // Background music generation with Lyria
                promptParts.push(`## REQUIRED: Generate Background Music with Lyria`, ``, `Generate original background music that matches the content mood using Google Lyria:`, `\`\`\`bash`, `python3 skills/asset-generation/scripts/lyria_music.py \\`, `  --prompt "YOUR MUSIC DESCRIPTION" \\`, `  --output <work_dir>/assets/clips/bgm.mp3`, `\`\`\``, ``, `**Music prompt tips:**`, `- Be specific: genre, tempo (BPM), mood, instruments`, `- Match the content emotion:`, isEn ? [
                ...
                ].join("\n"), `- Use \`google/lyria-3-clip-preview\` for 30s clips (default, good for short videos)`, `- Use \`--model google/lyria-3-pro-preview\` for longer tracks if needed`, ``, `**In the final ffmpeg mix**, layer the BGM under the narration:`, `- BGM volume should be ~20-30% of narration volume (use \`-filter_complex "[1:a]volume=0.25[bgm];[0:a][bgm]amix=inputs=2:duration=first"\`)`, `- Fade in BGM at start (2s) and fade out at end (3s)`, ``);
                promptParts.push(`## CRITICAL: Horizontal-to-Vertical Video Conversion`, `The final output MUST be 9:16 vertical (1080x1920). If any source clip is horizontal (wider than tall):`, ``, `**Strategy A (preferred): Full-screen crop — NO black bars**`, `\`ffmpeg -i input.mp4 -vf "crop=ih*9/16:ih:(iw-ih*9/16)/2:0,scale=1080:1920" ...\``, `Use this when the subject stays in the center and won't be cut off.`, ``, `**Strategy B: Width-match with vertical centering — subject too wide to crop**`, `\`ffmpeg -i input.mp4 -vf "scale=1080:-2,pad=1080:1920:0:(oh-ih)/2:black" ...\``, `This scales width to 1080, then pads top and bottom EQUALLY to center vertically.`, `The formula \`(oh-ih)/2\` is critical — it puts equal black bars on top and bottom.`, ``, `**VERIFY**: After producing the final video, extract a frame and confirm:`, `- No content is off-center vertically`, `- If black bars exist, they must be EQUAL top and bottom`, `- Subject is not cropped unless Strategy A was deliberately chosen`, `\`ffmpeg -i final.mp4 -ss 3 -frames:v 1 -y /tmp/verify.png\``, ``, `## REQUIRED: Generate Publishing Copytext & Tags`, `After producing the final video, you MUST also generate a publishing copytext file.`, `Write it to \`output/copytext.md\` in the work directory.`, ``, `The copytext MUST follow viral/爆款 principles:`, `- **Hook line (first sentence)**: Must grab attention in under 2 seconds of reading. Use curiosity gaps, bold claims, or relatable pain points.`, `- **Body (2-3 sentences max)**: Expand on the hook, add value or intrigue. Keep it conversational and platform-native.`, `- **Call to action**: Encourage engagement (关注/收藏/转发/评论). Be natural, not pushy.`, `- **Tags/Hashtags**: Include 5-10 relevant hashtags. Mix:`, `  - 2-3 high-traffic trending tags (热门标签)`, `  - 2-3 niche/topic-specific tags`, `  - 1-2 branded or unique tags`, `  - Format: #tag1 #tag2 #tag3 (each prefixed with #)`, ``, `Example format of copytext.md:`, `\`\`\``, `这个方法我后悔没早点知道...`, ``, `很多人不知道，其实只要掌握这个技巧就能轻松搞定。今天一次性讲清楚，看完直接上手！`, ``, `觉得有用就收藏起来，别划走了 👆`, ``, `#知识分享 #干货 #涨知识 #教程 #生活技巧`, `\`\`\``, ``, `The copytext language should match the target platform (Chinese for 抖音/小红书).`, `Tailor the tone to the content category and platform style.`);
            }
            ...
            const emotionDirective = emotionMap[work.contentCategory];
            if (emotionDirective) {
                promptParts.push(``, `## IMPORTANT: Target emotion for this content is ${emotionDirective}`);
            }
            ...
            if (step === "assets" && work.type === "image-text") {
                ...
                const method = assetMethod[work.contentCategory];
                if (method)
                    promptParts.push(method);
            }
        }
...
apiRoutes.post("/api/works/:id/pipeline/advance", async (c) => {
    ...
    try {
        ...
        if (!completedStep)
            return c.json({ error: "completedStep is required" }, 400);
        ...
        const titleUpdate = { pipeline: work.pipeline };
        ...
        if (completedStep) {
            loadStepHistory(id, completedStep).then(history => {
                ...
                if (h?.blocks) {
                    getWork(id).then(w => {
                        syncStepConversation(id, w?.title ?? "Untitled", completedStep, w?.pipeline?.[completedStep]?.name ?? completedStep, h.blocks).catch(() => { });
                    }).catch(() => { });
                }
            }).catch(() => { });
        }
...
apiRoutes.get("/api/works/:id/steps/:step/history", async (c) => {
    ...
    if (!history)
        return c.json({ error: "No history for this step" }, 404);
    return c.json(history);
});
...
apiRoutes.get("/api/memory/search", async (c) => {
    ...
    if (!client)
        return c.json({ error: "Memory not configured (missing apiKey)" }, 503);
    const q = c.req.query("q") ?? "";
    if (!q)
        return c.json({ error: "Missing query parameter ?q=" }, 400);
    const method = (c.req.query("method") ?? "hybrid");
...
apiRoutes.get("/api/memory/context/:workId", async (c) => {
    ...
    const platform = typeof firstPlatform === "string" ? firstPlatform : firstPlatform?.platform ?? "通用";
