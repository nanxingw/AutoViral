...
// ─── PRD-0011 F2 — POST /comp/fps (canvas-fps switch) ────────────────────────
// Routes through the shared `ops.setFps` (the SAME code the Studio TweaksPanel
// fps control runs), so the agent CLI (`autoviral comp fps 24`) and the human
// UI converge. A valid fps writes comp.fps + broadcasts composition-changed;
// an invalid fps is rejected 400 + code:4 with disk untouched (no broadcast).
describe("bridge router — F2 POST /comp/fps", () => {
  let workRoot: string;
  const workId = "w_fps";
  const prevWorksRoot = process.env.AUTOVIRAL_WORKS_ROOT;
  ...
  beforeAll(async () => {
    const { mkdtemp, readFile, writeFile, mkdir } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    workRoot = await mkdtemp(join(tmpdir(), "autoviral-fps-route-"));
    const fixtureYaml = (
      await readFile(
        join(__dirname, "../../../../tests/fixtures/sample-work/composition.yaml"),
        "utf8",
  ...
  it("POST /comp/fps writes comp.fps and the next GET reflects it", async () => {
    const res = await app.request("/api/bridge/v1/comp/fps", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-AutoViral-Work-Id": workId,
  ...
  it("POST /comp/fps broadcasts composition-changed after the write lands", async () => {
    const events: string[] = [];
    const off = uiEventBus.subscribe(workId, (e) => events.push(e.type));
    try {
      const res = await app.request("/api/bridge/v1/comp/fps", {
  ...
  it("POST /comp/fps with an invalid fps → 400 + code 4, disk UNCHANGED, no broadcast", async () => {
    const { readFile } = await import("node:fs/promises");
    const target = join(workRoot, workId, "composition.yaml");
    const before = await readFile(target, "utf8");
    const events: string[] = [];
  ...
  it("POST /comp/fps without the work-id header → 400 + code 4", async () => {
    const res = await app.request("/api/bridge/v1/comp/fps", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fps: 24 }),
