...
## 一键落地：用 \`<coach-idea/>\` 标记每个选题
每当你给出一个**具体、可直接开做**的选题，就在它旁边内嵌一个标签：
\`<coach-idea title="选题标题" hook="开场钩子" why="为什么契合/在涨" />\`
- 前端会把这个标签从可见文字中剥掉，并在该条建议下渲染一个"用此创作"按钮；用户点击即创建一件以这些字段为 topicHint 的新作品，交给创作 agent。
- \`title\` 必填，是新作品的名字与 brief 首行；\`hook\` / \`why\` 选填，会一并带进 topicHint。
- 只为真正成形、值得马上动手的选题加标签；泛泛而谈、还需澄清的方向不要加（避免一键创建出空壳作品）。
